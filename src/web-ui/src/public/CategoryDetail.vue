...
export default {
  ...
  mounted() {
    ...
    this.listener = () => {
      const collapseElements = this.showGenderFilter
        ? [this.$refs.genderCollapse, this.$refs.styleCollapse]
        : [this.$refs.styleCollapse];
  ...
  methods: {
    ...
    async getProductsByCategory(categoryName) {
      ...
      if (this.personalizeUserID && intermediate.length > 0) {
        const response = await RecommendationsRepository.getRerankedItems(this.personalizeUserID, intermediate);
    ...
    filteredProducts() {
      ...
      if (selectedGenders.length) {
        products = products.filter(
          (product) => selectedGenders.includes(product.gender_affinity) || !product.gender_affinity,
        );
      }
      return products;
    },
